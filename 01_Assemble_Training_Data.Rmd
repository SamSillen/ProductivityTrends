...
usa <- us_states() %>%
  st_transform(2163) %>%
  filter(state_abbr %in% c("WY", "ID", "MT", "CO", "UT"))

# plot matchup sites
ggplot() +
  geom_sf(data=usa, color="blue", fill="white") +
...
2) Pull NHD COMIDS from Intermountain West states, join to join to the aquasat roi via st_intersects
```{r}
...
rm(df.surf_temp, lagos_roi, lakeCat, lakeCat.full, lakeCat.shrunk, lakes, lc, nhd_paths, roi, lc.files, tday, tdstr, tmonth, tunits, tustr, tyear, nc_dat, round.1, round1, states)
```
...
write.csv(training, "C:/Users/samsi/Desktop/WesternMountainsChlPreds/Data/training.csv", row.names  = FALSE)
