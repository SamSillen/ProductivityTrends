...
ut_comids <- get_waterbodies(AOI = ut_area, id = NULL, t_srs = NULL, buffer = 0.5) ut_comids %>%
    filter(ftype == 'LakePond' | ftype == 'Reservoir')
...
NHD_Lagos <- st_join(all_comids, west_lagos, join = st_nearest_feature)
...
training$year_month <- format(training$date, %Y-%m")
