...
1) Read in training file ; filter, calculate band ratios, and remove correlated variables 
```{r}
training <- read.csv("C:/Users/samsi/Dropbox/WesternMountainsChlPreds/csvs/training.csv") 
...
#model_stats.table <- as.table(cv$byClass) 

#model_stats.table <- t(model_stats.table)

#colnames(model_stats.table)[1] <- "Eutrophic"

#colnames(model_stats.table)[2] <- "Mesotrophic"

#colnames(model_stats.table)[3] <- "Oligotrophic"

#model_stats.table <- format(model_stats.table, scientific = FALSE, digits = 2)%>%
#  addHtmlTableStyle(css.header = "padding: 10px", css.cell = "padding:3px")

#model_stats.table <- htmlTable(model_stats.table)

#save_kable(cv, "C:/Users/samsi/Dropbox/WesternMountainsChlPreds/model_stats.html")
...
ggplot(plot, aes(Prediction,Reference, fill= Frequency)) +
        ...
        theme_bw() +
        theme(axis.text.x=element_text(size=15, family = 'sans'), axis.text.y = element_text(size=15, family = 'sans'), axis.title.x =               element_text(size=20, face = 'bold', family = 'sans'),axis.title.y = element_text(size=20, face = 'bold', family = 'sans')) 
